...
\begin{figure}[t]
\centering
\makebox[\linewidth][c]{\includegraphics[width=1.04\linewidth]{evidence_importance.pdf}}
...
The replication artifact is available at \url{https://github.com/PhantomPhoenixIN/EviCode}. The repository contains the current manuscript source, configuration files, extraction and analysis scripts, probe-basis audit code, baseline and learner-ablation scripts, bibliography files, and figure artifacts used by the paper. Large intermediate outputs such as frozen embeddings and fitted models can be regenerated from the documented pipeline. Seed 42 is fixed, and labels, execution outcomes, compiler outcomes, generators, and problem identifiers are excluded from the feature matrix.
